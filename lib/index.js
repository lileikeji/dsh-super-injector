...
export function apply(ctx, config) {
    ...
    function hasActiveEntry(pkgName) {
        for (const entry of ctx.loader.entries()) {
            ...
            if (opts.group)
                continue;
            if (opts.name === pkgName && entry.fiber && FIBER_NAMES[entry.fiber.state] === 'active')
                return true;
        }
        return false;
    }
    ...
    function matchesSelf(match) {
        if (String(match).includes('dsh-super-injector'))
            return true;
        for (const entry of ctx.loader.entries()) {
            const o = entry.options;
            if (o.group)
                continue;
            if (!String(o.name).includes('dsh-super-injector'))
                continue;
                ...
                void (async () => {
                    try {
                        ...
                        if (attempt < 3)
                            ...
                            auditLog('heal-exhausted', '3 次均失败（需人工介入：修复产物后 touch profile patch 触发重装配）');
                    }
                })();
            };
            globalThis.setTimeout(heal, 4000);
    ...
    async function reloadPackage(match, urlMatch) {
        ...
        if (!internal)
            return 'ERROR: loader.internal 不可用';
        ...
        if (urls.length === 0) {
            ...
            try {
                ...
                if (urlMatch)
                    candidates.push(join(urlMatch, 'lib', 'index.js'));
        ...
        if (!entryUrl)
            return `INFO: 缓存中无匹配且磁盘降级失败 "${urlKey}" 的模块`;
            ...
            const restorePrecheckCache = () => {
                for (const [u, job] of precheckBackup) {
                    if (job === undefined)
                        ...
                        Map.prototype.set.call(loadCache, u, job);
                }
            };
            try {
        ...
        try {
            ...
            if (cmDbg?.table) {
                const keys = [];
                for (const k of cmDbg.table.keys())
                    keys.push(String(k));
    ...
    /** staging 工具不得在宿主 ctx 上注册事件监听：异常会异步炸掉整个 turn。 */
    function stagingSourceBlocked(source) {
        if (typeof source !== 'string')
            return false;
        return /(?:\bctx\s*(?:\.events)?\s*\.\s*(?:on|once)\s*\(|\bctx\s*(?:\.events)?\s*\[\s*['"]\s*(?:on|once)\s*['"]\s*\]\s*\(|\b(?:on|once)\b[^;]*=\s*ctx\b)/.test(source);
    }
    ...
    function restoreStaging() {
        try {
            ...
            for (const [name, raw] of Object.entries(data)) {
                ...
                if (staged.has(name))
                    continue;
                if (stagingSourceBlocked(raw.source)) {
                    logger.warn('[super-injector] staging 跳过危险源码 %s（禁止在宿主 ctx 上注册事件监听）', name);
                    continue;
                }
                let fn;
                try {
                ...
                if (tool.promoted) {
                    try {
                        const dispose = ctx.effect(() => ctx.tools.register(defineTool({
                            ...
                            output: { schema: { type: 'string' }, render: (_x, v) => [{ type: 'text', text: String(v) }] },
                            async execute(args) {
    ...
    safeRegister(defineTool({
        ...
        async execute(args) {
            ...
            if (stagingSourceBlocked(args.execute))
                return 'ERROR: staging 禁止注册 ctx.on / ctx.events.on 事件监听（会在宿主 ctx 异步运行并可能让整个 turn 报错）。需要真实事件钩子请走 dev_inject_plugin / dev_install_package 插件装配。';
            let fn;
            try {
    ...
    safeRegister(defineTool({
        ...
        async execute(a) {
            ...
            if (stagingSourceBlocked(t.source))
                return `ERROR: ${a.name} 含被禁止的 ctx.on 事件监听，已拒绝执行`;
            try {
    ...
    safeRegister(defineTool({
        ...
        async execute(a) {
            const t = staged.get(a.name);
            if (!t)
                return `ERROR: staging 无此工具（${a.name}）`;
            if (t.promoted)
                return `${a.name} 已在前侧（转正过）`;
            try {
    ...
    function waitFiberStable(entry, timeoutMs = 3000) {
    ...
    function normalizeEntriesByName(name) {
        for (const entry of ctx.loader.entries()) {
            ...
            if (o.group)
                continue;
            if (o.name === name || o.name.includes(name))
    ...
    function clientStatus(name) {
        ...
        if (!declared)
            return 'client 跳过（无 client 声明，属预期）';
        // 2. 有声明 → 查注册表
        try {
            const cm = ctx.get('clientModules');
    ...
    function arbitrateOfficial() {
        try {
            ...
            if (!official || !official.disabled || ghosts.length === 0)
                return;
    ...
    async function inject(dir) {
        ...
        if (hasActiveEntry(pkgName))
            return `INFO: ${pkgName} 已激活运行，跳过注入`;
        purgeCache(absDir);
    ...
    async function uninject(match) {
        ...
        if (entryUnloaded)
            recordOp('uninject', true);
        else if (!noMatch)
            recordOp('uninject', false);
        return 'OK: 卸载完成\n- ' + steps.join('\n- ');
    }
    ...
    function buildFreshnessProblems(base) {
        ...
        try {
            ...
            const hasClient = !!pkg.dsh?.client;
    ...
    ctx.setInterval(() => {
        ...
        for (const w of watchList) {
            ...
            if (String(w.match).includes('dsh-super-injector'))
                continue;
            ...
            if (prev !== undefined && prev !== fp) {
                ...
                const precheck = existsSync(libFile)
                    ? ctx.loader.import(pathToFileURL(libFile).href, () => [])
                        .then((ns) => { const probe = ctx.loader.unwrapExports(ns); return !!(probe && (typeof probe === 'function' || typeof probe.apply === 'function')); })
                        .catch(() => false)
                    : Promise.resolve(false);
                precheck.then((probeOk) => {
                    if (!probeOk) {
                        reloading = false;
            ...
            for (const name of names) {
                try {
                    if (typeof toolsSvc.unregister === 'function')
                        ...
                        Map.prototype.delete.call(table, name);
                }
                catch { /* 单项清理失败继续 */ }
            }
            if (names.length > 0) {
    ...
    safeRegister(defineTool({
        ...
        async execute() {
            const kinds = ['inject', 'reload', 'uninject', 'install', 'selfHeal'];
    ...
    function detectCheckout() {
        ...
        if (env && existsSync(join(env, 'packages')))
            return env;
        const candidates = [
    ...
    safeRegister(defineTool({
        ...
        async execute(args) {
            ...
            if (!targetDir || !rawName)
                return 'ERROR: dir 与 name 必填';
    ...
    safeRegister(defineTool({
        ...
        async execute(args) {
            ...
            if (existsSync(join(pluginDir, 'scripts', 'build.sh'))) {
                const bash = findBash();
                if (!bash)
                    return 'ERROR: bash 不可用（PATH 与常见路径均未找到）';
    ...
    safeRegister(defineTool({
        ...
        async execute(args) {
            ...
            if (!repo)
                return 'ERROR: 无法推断仓库（git remote origin）';
    ...
    safeRegister(defineTool({
        ...
        async execute() {
            ...
            try {
                ...
                const beforeUid = (() => { for (const e of ctx.loader.entries()) {
                    const o = e.options;
                ...
                try {
                    ...
                    if (lcT) {
                        for (const u of [...lcT.keys()]) {
                            if (typeof u === 'string' && decodeURIComponent(u).includes(TEST_SHORT))
                ...
                const afterUid = (() => { for (const e of ctx.loader.entries()) {
                    const o = e.options;
                    if (!o.group && String(o.name) === TEST_PKG && e.fiber)
                        return e.fiber.uid;
                } return null; })();
                ...
                try {
                    ...
                    if (!libPath) {
                        ...
                        for (const u of lcT?.keys() ?? []) {
                            ...
                            if (!d.includes('dsh-super-injector') && !d.includes('cloud-restore'))
                                continue;
                ...
                for (const e of ctx.loader.entries()) {
                    const o = e.options;
                    if (!o.group && String(o.name) === TEST_PKG)
                        entryGone = false;
                }
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                if (req.method === 'POST' && path === '/uninstall') {
                    ...
                    if (!match)
                        return send(400, { ok: false, error: 'match 必填' });
                ...
                if (req.method === 'POST' && path === '/inject') {
                    ...
                    if (!dir)
                        return send(400, { ok: false, error: 'dir 必填' });
                ...
                if (req.method === 'POST' && path === '/ingest') {
                    ...
                    const dir = String(body?.dir ?? '').trim();
                    if (!dir)
                        return send(400, { ok: false, error: 'dir 必填（要内化成插件的文件夹路径）' });
