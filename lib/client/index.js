...
.spi-search{width:100%;box-sizing:border-box;margin-top:10px;background:var(--theme-input-bg,#111);color:var(--theme-text,#ddd);border:1px solid var(--theme-border,#333);border-radius:6px;padding:6px 8px;font-size:12px}
...
function SuperInjectorPluginsPanel() {
    ...
    useEffect(() => {
        ...
        page.append(list);
        const search = el('input', 'spi-search');
        search.type = 'search';
        search.placeholder = '搜索注入插件';
        page.append(search);
        ...
        const refresh = () => {
            fetchJson('/list')
                .then((d) => {
                if (!d?.ok)
                    return say(JSON.stringify(d), true);
                ...
                const shown = entries.filter((e) => (String(e.name) + ' ' + String(e.dir)).toLowerCase().includes(q));
                if (!shown.length) {
        ...
        const doAction = (path, label) => {
            ...
            fetchJson(path, { method: 'POST', body: JSON.stringify({ dir, title: label }) })
                .then((r) => { say(r?.result ?? JSON.stringify(r), !r?.ok); if (r?.ok)
...
export function apply(ctx) {
    ctx.effect(() => ctx.slots.inject('settings.section', () => ctx.slots.register({
        ...
        order: 50,
        label: () => '插件注入',
    }, SuperInjectorPluginsPanel)), 'super-injector: settings page');
}
